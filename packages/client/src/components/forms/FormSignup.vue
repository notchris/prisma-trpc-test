  ...
  <form @submit.prevent="signup()"
    ...
    <div class="block">
      <div class="flex items-center gap-2">
        ...
        <div title="Username must be 4-18 characters long.">
      ...
      <input type="text"
             @input="inputChanged"
             :class="[v$.username.$error ? 'border-red-500 focus:border-red-300 focus:ring-red-200' : 'border-gray-300 focus:border-indigo-300 focus:ring-indigo-200', 'mt-1 block w-full rounded-md shadow-sm focus:ring focus:ring-opacity-50']"
             name="username"
             v-model="v$.username.$model"
             required />
      <div class="py-2 text-xs text-red-500"
           v-if="v$.username.$error">Please enter a valid username.</div>
    </div>
...
export default defineComponent({
  ...
  validations() {
    return {
      ...
      username: { required, minLength: minLength(4), maxLength: maxLength(18) },
